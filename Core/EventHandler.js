...
module.exports = class EventHandler {
  ...
  async onMessage(Message){
    ...
    if(Message.channel.type !== "text" && Message.author.id !== this.client.config.owner) return Message.reply("You can't use commands in Private Messages.");
    ...
    try {
      ...
      if(await this.client.cmds.processCooldown(Message, cname)) {
        ...
        if(command.permissions.includes('auth') && user === null) return Message.reply(`You haven't authorized your Trello account with Discord! Please authorize here: https://api.snazzah-is.cool/trello/auth`)
